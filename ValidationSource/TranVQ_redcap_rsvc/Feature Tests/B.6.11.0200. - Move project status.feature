Feature: User Interface: The system shall support the ability for a user to change between following project statuses bidirectionally: Development, Production, Analysis/Cleanup, Mark as Complete.
    ...
    Scenario: B.6.11.0200.100 Move project from development to production to analysis/cleanup to complete to analysis/cleanup to production to development
        ...
        And I should see a table header and rows containing the following values in the logging table:
            ...
            | tranvq_adminuser | Manage/Design | Move project back to Development status                             |
            | tranvq_adminuser | Manage/Design | Return project to Production from Analysis/Cleanup status           |
            | tranvq_adminuser | Manage/Design | Project moved from Completed status back to Analysis/Cleanup status |
            | tranvq_adminuser | Manage/Design | Project marked as Completed                                         |
            | tranvq_adminuser | Manage/Design | Move project to Analysis/Cleanup status                             |
            | tranvq_adminuser | Manage/Design | Move project to Production status                                   |
#END